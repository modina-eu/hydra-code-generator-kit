import {
  ...
  sublanguageProp,
  syntaxTree
} from "./chunk-GJ6PWYVV.js";
import {
  styleTags,
  tags
} from "./chunk-TELWAZQG.js";
import {
  Decoration,
  Direction,
  EditorView,
  ...
  logException,
  showTooltip
} from "./chunk-WGMZIEMP.js";
import "./chunk-7F6KBBOE.js";
import {
  Annotation,
  CharCategory,
  EditorSelection,
  ...
  combineConfig,
  fromCodePoint
} from "./chunk-WGZ5GUV2.js";
import "./chunk-LQ2VYIYD.js";

// node_modules/@lezer/lr/node_modules/@lezer/common/dist/index.js
...
function resolveNode2(node, pos, side, overlays) {
  ...
  if (overlays)
    for (let scan = node, parent = scan.parent; parent; scan = parent, parent = scan.parent) {
      if (scan instanceof TreeNode2 && scan.index < 0 && ((_a = parent.enter(pos, side, mode)) === null || _a === void 0 ? void 0 : _a.from) != scan.from)
        node = parent;
    }
  for (; ; ) {
    let inner = node.enter(pos, side, mode);
...
var BaseNode2 = class {
  ...
  matchContext(context) {
    return matchNodeContext2(this, context);
  }
  enterUnfinishedNodesBefore(pos) {
    let scan = this.childBefore(pos), node = this;
    while (scan) {
...
var TreeNode2 = class _TreeNode extends BaseNode2 {
  ...
  nextChild(i, dir, pos, side, mode = 0) {
    for (let parent = this; ; ) {
      for (let { children, positions } = parent._tree, e = dir > 0 ? children.length : -1; i != e; i += dir) {
        let next = children[i], start = positions[i] + parent.from;
        if (!checkSide2(side, pos, start, start + next.length))
          continue;
        if (next instanceof TreeBuffer2) {
          ...
          let mounted;
          if (!(mode & IterMode2.IgnoreMounts) && (mounted = MountedTree2.get(next)) && !mounted.overlay)
            return new _TreeNode(mounted.tree, start, i, parent);
          let inner = new _TreeNode(next, start, i, parent);
          return mode & IterMode2.IncludeAnonymous || !inner.type.isAnonymous ? inner : inner.nextChild(dir < 0 ? next.children.length - 1 : 0, dir, pos, side);
        }
      }
...
function stackIterator2(tree, pos, side) {
  ...
  for (let scan = inner instanceof TreeNode2 ? inner : inner.context.parent; scan; scan = scan.parent) {
    if (scan.index < 0) {
      ...
      if (mount && mount.overlay && mount.overlay[0].from <= pos && mount.overlay[mount.overlay.length - 1].to >= pos) {
        let root = new TreeNode2(mount.tree, mount.overlay[0].from + scan.from, 0, null);
...
var TreeCursor2 = class {
  ...
  */
  matchContext(context) {
    if (!this.buffer)
      return matchNodeContext2(this.node, context);
    let { buffer } = this.buffer, { types } = buffer.set;
    for (let i = context.length - 1, d = this.stack.length - 1; i >= 0; d--) {
      if (d < 0)
        return matchNodeContext2(this.node, context, i);
      let type = types[buffer.buffer[this.stack[d]]];
      if (!type.isAnonymous) {
        if (context[i] && context[i] != type.name)
          return false;
...
var NodeWeakMap = class {
  ...
  */
  set(node, value) {
    if (node instanceof BufferNode2)
  ...
  */
  get(node) {
    return node instanceof BufferNode2 ? this.getBuffer(node.context.buffer, node.index) : node instanceof TreeNode2 ? this.map.get(node.tree) : void 0;
  }
  /**
